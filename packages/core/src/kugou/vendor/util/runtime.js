...
// [本项目改动] 原实现是 const { URL } = require('url')，取 Node 的 URL 类。
// React Native 的 Metro 打包器没有 'url' 这个内置模块，遇到该 require 会打包失败。
// 全局 URL 在 Node 和 RN 里都存在，改用它；两者都没有时降级为不支持代理。
const URLImpl = typeof URL === 'function' ? URL : null;
...
function resolveProxy() {
  ...
  if (!URLImpl) {
    console.warn('[proxy] 当前运行环境没有全局 URL，忽略代理配置');
    cachedProxy = null;
    return null;
  }
  try {
    const parsed = new URLImpl(rawProxy);
